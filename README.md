...
```
Total Loss = CrossEntropy + λ × mean(all gate values)
```
...
**Sparsity normalization matters more than it looks** — Switching the sparsity loss from `sum` to `mean` was the single change that made λ values interpretable and training stable. With `sum`, useful λ values were in the `1e-6` range and hard to reason about. With `mean`, λ = 1, 5, 20 produce clearly different pruning behaviors that match intuition.
...
- No warmup schedule for λ. Starting with a lower λ and ramping it up might produce a better sparsity-accuracy frontier.
...
- **Lambda warmup**: start λ at 0 and linearly increase it over the first few epochs to give the model time to form good representations before pruning pressure kicks in.
...
This trains three models (λ = 1.0, 5.0, 20.0), prints a results table, and saves the gate distribution plot to `outputs/gate_dist_all.png`.
